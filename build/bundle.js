...
// Listen for Submit
_ui.ui.postBtn.addEventListener('click', submitPost);
// Listen for Delete
_ui.ui.post.addEventListener('click', deletePost);
// Listen for Edit State
_ui.ui.post.addEventListener('click', enableEdit);
// Using event delegation because post-cancel btn is not add when the dom is loaded
_ui.ui.cardForm.addEventListener('click', cancelEdit);
...
function submitPost() {
    // Getting values
    var title = _ui.ui.titleInput.value;
    var body = _ui.ui.bodyInput.value;
    var id = _ui.ui.idInput.value;
    ...
    if (title !== '' && body !== '') {
        ...
        if (id !== '') {
            ...
            _http.http.post('http://localhost:3000/posts', data).then(function () {
                getPosts();
                _ui.ui.showAlert('Post Enviado', 'alert alert-success');
                _ui.ui.clearFields();
            }).catch(function (err) {
                return console.log(err);
            });
            ...
            // Update Post
            _http.http.put('http://localhost:3000/posts/' + id, data).then(function () {
                getPosts();
                _ui.ui.showAlert('Post Atualizado', 'alert alert-warning');
        ...
        _ui.ui.showAlert('Seu Post precisa ter Titulo e Conteúdo', 'alert alert-danger');
    }
}
    ...
    if (event.target.parentElement.classList.contains('delete')) {
        ...
        if (confirm('Você está certo disso?')) {
            _http.http.delete('http://localhost:3000/posts/' + id).then(function () {
                _ui.ui.showAlert('Post Removido com Sucesso', 'alert alert-info');
                getPosts();
            }).catch(function (err) {
                console.log(err);
            });
...
function enableEdit(event) {
    if (event.target.parentElement.classList.contains('edit')) {
        ...
        var title = event.target.parentElement.previousElementSibling.previousElementSibling.textContent;
        var body = event.target.parentElement.previousElementSibling.textContent;
...
// Making the EasyHTTP class exportable
...
var UI = function () {
    ...
    _createClass(UI, [{
        ...
        value: function clearAlert() {
            if (document.querySelector('.alert')) {
                $('.alert').fadeOut(2000, "linear", function () {
        ...
        key: 'changeFormState',
        value: function changeFormState(state) {
            if (state === 'edit') {
